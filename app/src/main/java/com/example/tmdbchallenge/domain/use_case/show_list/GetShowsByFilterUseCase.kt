...
class GetShowsByFilterUseCase @Inject constructor(
    private val dispatchers: DispatcherProvider,
    private val showRepository: ShowRepository,
) {

    suspend operator fun invoke(
        showFilter: ShowFilter = ShowFilter.TOP_RATED,
        page: Int = 1
    ): Flow<Resource<List<Show>>> {
        return flow {
            emit(Resource.Loading(true))

            val filteredResult = showRepository.getShowsByFilter(showFilter, page)
